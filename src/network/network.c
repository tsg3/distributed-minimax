...
void* master_network_main()
{
        ...
        for (int j = 0; j < 3; j++)
        {
            if (BIT_CHECK(worker_status, j) == 1)
            {
                ...
                if (flag)
                {
                    ...
                    json_to_resp(buffer, &value, &level);
                    free(buffer);

                    /** ------------------------------------------------------ */

                    add_response(value, level);
                }
            }
        }
        pthread_mutex_unlock(&lock);
    ...
    MPI_Send(&final_message, 1, MPI_BYTE, 
...
void* worker_minimax_main()
{
    while (1)
    {
        ...
        if (request_buffer != NULL)
        {
            ...
            State* state_recv;
            int level_recv;
            json_to_data(request_buffer, &state_recv, &level_recv); 
            /** -------------------- */
            ...
            // int res = calc_level(state_recv, level_recv);
            int res = 0; // TEST
            // char res_buffer[3];
            // snprintf(res_buffer, 3, "%d", res);
            // response_len = strlen(res_buffer);

            pthread_mutex_lock(&lock);

            /** Parse message to char array */
            response_buffer = resp_to_json(res, level_recv);
            response_len = strlen(response_buffer);
            // snprintf(response_buffer, response_len, "%d", res); // Here, 'request' should be the result.
            /** --------------------------- */
            worker_state = '1';
            pthread_mutex_unlock(&lock);

            delete_state(state_recv);
        }
